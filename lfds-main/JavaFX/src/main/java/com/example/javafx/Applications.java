...
public class Applications extends Application {
    ...
    public void show_scene_1() {
        ladebalken = new ProgressBar();
        Button updateButton = new Button("update");
        updateButton.setOnAction(e -> update());
        Button nextButton = new Button("Diagramme anzeigen");
        nextButton.setOnAction(e -> next());

        VBox root = new VBox(10);
        root.setPadding(new Insets(10));
        root.getChildren().addAll(ladebalken, updateButton, nextButton);

        stage.setScene(new Scene(root, 200, 120));
        stage.setTitle("Ladebalken App");
        stage.show();
    }
        ...
        public void run() {
            try {
                ...
                while ((value = inputStream.read()) != -1) {
                    System.out.println("Wert empfangen: " + value);
                }

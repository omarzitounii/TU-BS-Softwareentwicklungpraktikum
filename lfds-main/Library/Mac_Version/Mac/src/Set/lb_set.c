 ...
 * Initialization in benchmark.c should be like that: lf_stack_node_t* tree = NULL;
 * by insert: tree = lb_set_insert(tree, j);
 * by delete: tree = lb_set_delete(tree,j);
 */

/* set_search is not needed for benchmark
lf_stack_node_t *set_search(lf_stack_node_t *tree, int key) {
    lf_stack_node_t *current = tree;
    while (current != NULL)
    {
...
lb_set_node_t* lb_set_insert(lb_set_node_t* tree, int value)
{
    if (tree == NULL)
    {

        lb_set_node_t* newNode = (lb_set_node_t*)malloc(sizeof(lb_set_node_t));
        if (newNode == NULL){
            perror(malloc);
        }
        newNode->value = value;
    ...
    if (value < tree->value)
    {
        tree->left = lb_set_insert(tree->left, value);
    }
    else
    {
        tree->right = lb_set_insert(tree->right, value);
    }
    return tree;
}

lb_set_node_t* find_min(lb_set_node_t* node) {
...
lb_set_node_t* lb_set_delete(lb_set_node_t* tree, int key) {
    if (tree == NULL) {
        return NULL;
    }
    if (key < tree->value) {
        tree->left = lb_set_delete(tree->left, key);
    } else if (key > tree->value) {
        tree->right = lb_set_delete(tree->right, key);
    } else {
        // Key found, perform deletion

        if (tree->left == NULL && tree->right == NULL) {
            // Case 1: lf_stack_node_t has no children
            free(tree);
            return NULL;
        } else if (tree->left == NULL) {
            // Case 2: lf_stack_node_t has only right child
            lb_set_node_t* temp = tree->right;
            free(tree);
            return temp;
        } else if (tree->right == NULL) {
            // Case 3: lf_stack_node_t has only left child
            lb_set_node_t* temp = tree->left;
            free(tree);
            return temp;
        } else {
            // Case 4: lf_stack_node_t has both left and right children
            lb_set_node_t* min_right = find_min(tree->right);
            tree->value = min_right->value;
            tree->right = lb_set_delete(tree->right, min_right->value);
        }
    }
    return tree;
}
